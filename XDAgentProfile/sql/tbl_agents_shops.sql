...
CREATE TABLE IF NOT EXISTS `tbl_agents_shops` (
  ...
  `agentid` int(11) NOT NULL COMMENT '商家ID add new ',
  `contract_code` int(11) NOT NULL COMMENT '签约商管理的合同号',
  `shopid` int(11) NOT NULL COMMENT '签约商管理的店面id',
  `shopname` varchar(50) NOT NULL COMMENT '签约商管理的店面名称',
  `shopprovince` varchar(6) NOT NULL COMMENT '省份',
  ...
  `shoptype` varchar(45) NOT NULL COMMENT '店铺类型复选',
  `shopdesc` varchar(128) COMMENT '店铺描述'
  PRIMARY KEY (`id`)
